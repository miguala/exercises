...
resource "aws_dynamodb_table" "contacts8a" {
  ...
  stream_enabled   = true
  stream_view_type = "NEW_IMAGE"  # Habilitar Streams con la nueva imagen
}

# Crear el tema SNS (renombrado a Contacts8aTopic)
...
# Renombrar dynamodb_trigger a dynamodb8a_trigger
...
# Renombrar sns_trigger a sns8a_trigger
...
resource "aws_lambda_permission" "api_gateway" {
  ...
  depends_on = [aws_lambda_function.create_contact8a]
}
